...
\begin{itemize}
    \item \textbf{Component} ({\tt User}): definisce l'interfaccia per gli oggetti a cui verranno aggiunte proprietà in modo dinamico.
    \item \textbf{ConcreteComponent} ({\tt UtenteBase}): definisce l'oggeto su cui verranno dinamicamente aggiunte le proprietà.
    \item \textbf{Decorator}({\tt Ruolo}): mantine una referenza al {\em Component} e definisce un'interfaccia conforme a quella del {\em Component}.
    \item \textbf{ConcreteDEcorator} ({\tt Dipendente, Cliente}): aggiunce le responsabilità al {\em Component}.
\end{itemize}
