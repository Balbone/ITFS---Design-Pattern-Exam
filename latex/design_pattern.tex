...
Per implementare il patter Decorator abbiamo bisogno delle seguenti componenti (cfr.\cite{gof_sunt}, p.65):
\begin{itemize}
    ...
    \item Uno o più {\em ConcreteDecorator} ({\tt Dipendente, Cliente}): aggiunce le responsabilità al {\em Component}.
\end{itemize}
La relazione tra le componenti del decorator è illustrata sotto forma di UML diagram in fig. \ref{fig:uml_diagram_decorator}.

Nell'implementazione del decorator devo tenere conto di alcuni accorgimenti:
\begin{itemize}
    \item La conformità delle interfacce: l'interfaccia di un oggetto decorato deve essere conforme all'interfaccia del {\em Component} che decora. Di conseguenza, la classe {\em ConcreteDecorator} deve ereditare da una classe comune.
    \item Posso non definere la classe \textbf{astratta} {\em Decorator} se devo aggiungere una sola responsabilità, ma basta includere le responsabilità aggiuntive del {\em Decorator} nel {\em ConcreteDecorator}. Questa situazione è tipica di quando ho a che fare con una gerarchia di classi già definita, piuttosto che una nuova da definire.
    \item è importante mantenere la classe {\em Component} ``leggera'', ossia: il suo scopo è la definizione di una interfaccia conforme, non deve contenere altri dati. La definizione dei dati rappresentativi è competenza delle sottoclassi, altrimenti la complessità della classe {\em Component} rende i {\em decorators} troppo pesanti per essere applicati ricorsivamente. Mettere troppe funzionalità nella classe {\em Component} aumenta la possibilità che le sottoclassi {\em concrete} paghino per caratteristiche di cui non hanno bisogno.
    \item Cambiare la pelle di un oggetto, non le viscere. Possiamo pensare al Decorator come a una pelle che ricopre l'oggetto e che ne cambia il comportamento. Se desideriamo un cambiamento più viscerale, la scelta giusta è il \textbf{pattern Strategy}.
\end{itemize}
...
Rispetto al Decorator, il patter Strategy è la scelta giusta quando la classe {\em Component} è contenutisticamente ``pesante'' (vedi commetti sopra), e quindi rende il patter Decorator troppo costoso da applicare.
Nel patter Strategy il {\em component} passa alcune delle sue caratteristiche a un oggetto {\em Strategy} separato \cite{3}.
